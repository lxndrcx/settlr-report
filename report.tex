...
\begin{defn}{TPTL Semantics}\label{tptlsem}
  ...
  TPTL uses the same semantics as LTL (\Cref{ltlsem}) for those operators which overlap with LTL, except the relation is defined over a time trace rather than a standard trace. For those cases, substitute occurrences of $\sigma\vDash_{\text{LTL}}\cdots$ with $\rho\vDash_{\text{TPTL},\eval}\cdots$. The remainder of the definition is as follows:
  \begin{align*}
...
In my implementation of RV-TPTL, I have made a modified and approximated the semantics. See \Cref{implementaion} for details.
