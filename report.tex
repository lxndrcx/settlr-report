...
\begin{document}
\lstset{language=Scala,basicstyle=\ttfamily}
\maketitle
...
\begin{defn}{IE-LTL semantics}
  ...
  Let $\sigma_f$ be the final state of the trace. Define the satisfaction relation for IE-LTL $\vDash_{\text{IE-LTL}}$ as follows. Copy the semantics from the LTL semantics (\Cref{ltlsem}), and add the following to them:
  \begin{align*}
...
\begin{defn}{IE-TPTL semantics}
  ...
  Let $\rho_f$ be the final state of the time trace. Define the semantic relation $\vDash_{\text{IE-TPTL}}$ of IE-TPTL as follows.Copy the semantics from the TPTL semantics (\Cref{tptlsem}), and add the following to them:
  \begin{align*}
...
My verifier is strutted as a library which can be integrated into other Scala programs.
...
def finaliseIE[A](state: (Int, A)): Const = {
  vprintln(2,s"finalise: $this @ time: ${state._1}, state: ${state._2}")
  def inner(f: Formula): Formula = {
    ...
    case Or(f1,f2) =>
      if (inner(f1) == True) True
      else Or(inner(f1),inner(f2)) apply state
    case And(f1,f2) =>
      if (inner(f1) == False) False
      else And(inner(f1),inner(f2)) apply state
    case fz @ Freeze(_, _) => inner(fz.subst(state._1))
    case lt @ LTEQ(_,_) => lt.substAll(state._1) apply state
    case _ => f apply state
    }
    vprintln(3,s"finalise.inner $f = $res")
    res
  }
  inner(this) match {
    case c:Const => vprintln(2,s"Finalise result: $c"); c
    case f => throw VerificationError(s"finalise.inner returns non-const: $f :(")
  }
}
\end{lstlisting}
...
\subsubsection{}
find where a timed formula changes from yes to no or whatever, in the infinite future or between states.

fill in previous times on command? can be implemented by just sending a new trace though right? How to make assumptions about what changes? interesting

\section{Conclusion}
\subsection{Acknowledgements}
I would like to thank Peter Baumgartner for supervising me and my wife Myvanwy for listening to me rant.
...
The \texttt{verifier} is entirely my work.
