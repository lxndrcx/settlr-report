...
The language is mostly defined using methods of the abstract class \lstinline{Formula}. Listing \ref{lstbasic} shows the definitions for LTL fragment:
\begin{lstlisting}[caption={Basic DSL Functions},label=lstbasic]
...
Instead, I was given access to a `progression calculator' for LTL that my supervisor had written previously. Using this as a base, I implemented my DSL to work with the provided progression functions which I then adapted to my project. The progression functions (\texttt{expand} and \texttt{unX})
